...
func TestIGroupLUNMap(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "Create lun map",
			input:            ClusterStr + "create lun map of lun named " + rn("/vol/vol1/lunpayroll") + " and an igroup named " + rn("igroupFinNew") + " on the " + rn("marketing") + " svm",
			expectedOntapErr: "",
			verifyAPI:        ontapVerifier{api: "api/protocols/san/lun-maps?igroup.name=" + rn("igroupFinNew") + "&lun.name=" + rn("/vol/vol1/lunpayroll") + "&svm.name=" + rn("marketing"), validationFunc: createObject},
		},
		{
			name:             "Clean lun map",
			input:            ClusterStr + "delete lun map of lun named " + rn("/vol/vol1/lunpayroll") + " and an igroup named " + rn("igroupFinNew") + " on the " + rn("marketing") + " svm",
			expectedOntapErr: "because it does not exist",
			verifyAPI:        ontapVerifier{api: "api/protocols/san/lun-maps?igroup.name=" + rn("igroupFinNew") + "&lun.name=" + rn("/vol/vol1/lunpayroll") + "&svm.name=" + rn("marketing"), validationFunc: deleteObject},
		},
		{
			name:             "Clean igroup",
			input:            ClusterStr + "delete igroup " + rn("igroupFinNew") + " on the " + rn("marketing") + " svm",

...
| `--tool-mode`       | Control which mutating tool naming convention is exposed. One of `legacy` (separate `update_*`/`delete_*` tools), `multiplex` (default - combined `modify_*` tools), or `both` (registers both conventions). Can also be set via the `TOOL_MODE` environment variable. <br/>  **Note:** `tool-mode` with value `multiplex` would reduce MCP tool count |
